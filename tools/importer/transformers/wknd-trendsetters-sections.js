 ...
 * Uses payload.template.sections from page-templates.json (7 sections).
 * Styled sections: rc1 (grey), rc3 (grey), rc5 (grey), rc7 (dark).
 ...
 * a marker <hr>. All selectors come from page-templates.json (DOM-verified).
 */
...
export default function transform(hookName, element, payload) {
  ...
  if (hookName === 'beforeTransform') {
    ...
    for (let i = sections.length - 1; i >= 0; i -= 1) {
      ...
      // First section with no style needs neither a break nor metadata.
      // (Here rc1 IS styled, so it still needs a marker for its metadata.)
